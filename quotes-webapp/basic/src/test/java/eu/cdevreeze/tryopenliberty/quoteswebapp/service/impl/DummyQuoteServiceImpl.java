...
public class DummyQuoteServiceImpl implements QuoteService {
    ...
    public Quote insertQuote(String quoteText, String attributedTo, ImmutableSet<String> subjects) {
        ImmutableList<Quote> updatedQuoteList = quotes.updateAndGet(quoteList -> {
            long nextId = 1 + quoteList.stream().mapToLong(Quote::quoteId).max().orElseThrow();
            Quote quote = new Quote(nextId, quoteText, attributedTo, subjects);
            return ImmutableList.<Quote>builder()
                    .addAll(quoteList)
                    .add(quote)
                    .build();
        });
        return updatedQuoteList.get(updatedQuoteList.size() - 1);
    }

    @Override
    public void deleteQuoteById(long quoteId) {
        quotes.updateAndGet(quoteList -> quoteList
                .stream()
                .filter(qt -> qt.quoteId() != quoteId)
                .collect(ImmutableList.toImmutableList()));
    }
